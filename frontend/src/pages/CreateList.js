...
const CreateList = () => {
    ...
    useEffect(() => {
        ...
        if (!location.state)
        {
            const fetchWIPList = async () => {
                ...
                {
                    // Fetching WIPList
                    const response = await fetch('http://107.21.101.56:4000/api/user/'+user.email+'/getWIP', {
                        headers: {
                ...
                else
                {
                    ...
                    const response = await fetch('http://107.21.101.56:4000/api/user/setWIP', {
                        method: "POST",
                        body: JSON.stringify({
                            email: user.email,
                            WIPList: newList
    ...
    const handleSaveListClick = async event => {
        ...
        const response = await fetch('http://107.21.101.56:4000/api/eventLists', {
                method: 'POST',
