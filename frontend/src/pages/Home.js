...
const Home = () => {
    ...
    const fetchEventLists = async () => {
            ...
            if (!browserCursor)
            {
                response = await fetch('http://107.21.101.56:4000/api/eventLists');
            }
    ...
    const fetchUserLists = async () => {
            ...
            const response = await fetch('http://107.21.101.56:4000/api/eventLists/user', {
                headers: {
    ...
    const fetchSavedLists = async () => {
            ...
            const response = await fetch('http://107.21.101.56:4000/api/user/'+jwtDecode(userToken.token).email+'/savedLists', {
                headers: {
                    'Authorization': `Bearer ${userToken.token}`
                    }
            });
